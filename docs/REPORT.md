...
### 환경 확인
```
**user@linux-machine**:**~**$ cat /etc/os-release
...
LOGO=ubuntu-logo
**user@linux-machine**:**~**$ uname -m
architecture
**user@linux-machine**:**~**$ whoami
user
**user@linux-machine**:**~**$ hostname -I
hostname
**user@linux-machine**:**~**$
```
...
```
user@linux-machine:~$ systemctl status ssh
Unit ssh.service could not be found.
```
SSH 서버가 없으므로 먼저 Ubuntu에 SSH 서버를 설치해야 한다.
```
user@linux-machine:~$ sudo apt update
user@linux-machine:~$ sudo apt install -y openssh-server
```
`sudo apt update`
...
### 1.4 검증
```
user@linux-machine:~$ sudo ss -tulnp | grep ssh
...
```
user@linux-machine:~$ sudo sshd -T | grep -E '^(port|permitrootlogin)'
port 20022
permitrootlogin no
user@linux-machine:~$ 
```
...
```
user@linux-machine:~$ grep -E '^(Port|PermitRootLogin)' /etc/ssh/sshd_config
Port 20022
PermitRootLogin no
user@linux-machine:~$ 
```
...
```
user@linux-machine:~$ ufw version
...
user@linux-machine:~$ sudo apt install -y ufw
user@linux-machine:~$ 
```
...
```
user@linux-machine:~$ sudo ufw status
Status: active
...
### 3.2 그룹 생성
```
user@linux-machine:~$ sudo groupadd agent-common
user@linux-machine:~$ sudo groupadd agent-core
```

### 3.3 계정 생성
계정 생성
```
user@linux-machine:~$ sudo useradd -m agent-admin
user@linux-machine:~$ sudo useradd -m agent-dev
user@linux-machine:~$ sudo useradd -m agent-test
```
- `-m`: 홈 디렉토리 생성. 홈 디렉토리를 생성하면 계정별 설정 파일도 자연스럽게 저장할 수 있다.

각 그룹에 계정 포함
```
user@linux-machine:~$ sudo usermod -aG agent-common agent-admin
user@linux-machine:~$ sudo usermod -aG agent-commont agent-dev
user@linux-machine:~$ sudo usermod -aG agent-common agent-dev
user@linux-machine:~$ sudo usermod -aG agent-common agent-test
user@linux-machine:~$ sudo usermod -aG agent-core agent-admin
user@linux-machine:~$ sudo usermod -aG agent-core agent-dev
```
...
```
user@linux-machine:~$ sudo passwd agent-admin
New password: 
Retype new password: 
passwd: password updated successfully
user@linux-machine:~$ sudo passwd agent-dev
New password: 
Retype new password: 
passwd: password updated successfully
user@linux-machine:~$ sudo passwd agent-test
New password: 
Retype new password: 
passwd: password updated successfully
user@linux-machine:~$ 
...
### 3.5 디렉토리 구조 설정
```
user@linux-machine:~$ sudo mkdir -p /home/agent-admin/agent-app/upload_files
user@linux-machine:~$ sudo mkdir -p /home/agent-admin/agent-app/api_keys
user@linux-machine:~$ sudo mkdir -p /home/agent-admin/agent-app/bin
user@linux-machine:~$ sudo mkdir -p /var/log/agent-app
```

### 3.6 디렉토리 구조 출력
```
user@linux-machine:~$ sudo tree -a /home/agent-admin/agent-app
...
```
user@linux-machine:~$ sudo setfacl -m g:agent-common:--x /home/agent-admin
```
...
```
user@linux-machine:~$ sudo chown agent-admin:agent-core /home/agent-admin/agent-app
user@linux-machine:~$ sudo chmod 750 /home/agent-admin/agent-app
user@linux-machine:~$ sudo setfacl -m g:agent-common:--x /home/agent-admin/agent-app
```
...
```
user@linux-machine:~$ sudo chown agent-admin:agent-common /home/agent-admin/agent-app/upload_files
user@linux-machine:~$ sudo chmod 2770 /home/agent-admin/agent-app/upload_files
user@linux-machine:~$ sudo setfacl -m d:g:agent-common:rwx /home/agent-admin/agent-app/upload_files
user@linux-machine:~$ sudo setfacl -m d:m:rwx /home/agent-admin/agent-app/upload_files
```
...
```
user@linux-machine:~$ sudo chown agent-admin:agent-core /home/agent-admin/agent-app/api_keys
user@linux-machine:~$ sudo chmod 2770 /home/agent-admin/agent-app/api_keys
user@linux-machine:~$ sudo setfacl -m d:g:agent-core:rwx /home/agent-admin/agent-app/api_keys
user@linux-machine:~$ sudo setfacl -m d:m:rwx /home/agent-admin/agent-app/api_keys
```
- 소유자:그룹을 `agent-admin:agent-core`로 설정
...
```
user@linux-machine:~$ sudo chown agent-admin:agent-core /var/log/agent-app
user@linux-machine:~$ sudo chmod 2770 /var/log/agent-app
user@linux-machine:~$ sudo setfacl -m d:g:agent-core:rwx /var/log/agent-app
user@linux-machine:~$ sudo setfacl -m d:m:rwx /var/log/agent-app
```
- 소유자:그룹을 `agent-admin:agent-core`로 설정
...
user@linux-machine:~$ sudo getfacl /home/agent-admin/agent-app
getfacl: Removing leading '/' from absolute path names
...
user@linux-machine:~$ sudo getfacl /home/agent-admin/agent-app/upload_files
getfacl: Removing leading '/' from absolute path names
...
user@linux-machine:~$ sudo getfacl /home/agent-admin/agent-app/api_keys
getfacl: Removing leading '/' from absolute path names
...
user@linux-machine:~$ sudo getfacl /var/log/agent-app
getfacl: Removing leading '/' from absolute path names
...
user@linux-machine:~$ ls -l /home/yejoo031053822/agent-app
...
```
user@linux-machine:~$ sudo mv /home/yejoo031053822/agent-app /home/agent-admin/agent-app/agent-app
user@linux-machine:~$ sudo chown agent-admin:agent-core /home/agent-admin/agent-app/agent-app
user@linux-machine:~$ sudo chmod 750 /home/agent-admin/agent-app/agent-app
...
### 4.7 포트 LISTEN 상태 확인
```
user@linux-machine:~$ sudo ss -tulnp | grep 15034
...
### 6.2 설정 파일 생성
```
user@linux-machine:~$ sudo vim /etc/logrotate.d/agent-app-monitor
```
...
### 6.3 현재 설정 파일 내용
```
user@linux-machine:~$ cat /etc/logrotate.d/agent-app-monitor
